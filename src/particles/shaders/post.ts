...
void main() {
  ...
  /* Out of linear, once, here. Doing it earlier would have the bloom looking
     for bright pixels in the wrong space. */
  vec3 encoded = mix(
    colour * 12.92,
    1.055 * pow(max(colour, vec3(0.0)), vec3(1.0 / 2.4)) - 0.055,
  ...
  fragColor = vec4(encoded, presence);
}
`;
